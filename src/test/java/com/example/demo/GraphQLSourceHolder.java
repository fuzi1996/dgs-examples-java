...
import static com.example.demo.BusinessServiceClient.batchSellerInfoByIds;
import static com.example.demo.BusinessServiceClient.getSellerInfoById;
import static com.example.demo.ConsumerServiceClient.batchUserInfoByIds;
...
public class GraphQLSourceHolder {
    ...
    private static DataFetcher userInfoListQueryDataFetcher = environment -> {
        Map<String, Object> arguments = environment.getArguments();
        Map<Object, Object> userInfoListQuery = (Map<Object, Object>) arguments
            .get("userInfoListQuery");
        String clientVersion = (String) arguments.getOrDefault("clientVersion", "defaultVersion");
    ...
    private static DataFetcher commodityDataFetcher = environment ->
        CommodityServiceClient
            .getItemBaseInfoById((Integer) environment.getArguments().get("itemId"));
    ...
    private static DataFetcher couponDataFetcher = environment ->
        MarketingServiceClient
            .getCouponInfoById((Integer) environment.getArguments().get("couponId"));

    private static DataFetcher abInfoDataFetcher = environment ->
        environment.getArgumentOrDefault("userId", -1) % 7;
    ...
    public static GraphQLSchema getDefaultSchema() {
        ...
        for (Map.Entry<String, Map<String, DataFetcher>> entry : defaultDataFetcherInfo()
            .entrySet()) {
    ...
    public static GraphQLSchema configGraphQLSchema(String schemaPath,
        Map<String, Map<String, DataFetcher>> dataFetcherConfig) {
        RuntimeWiring.Builder runtimeWiring = RuntimeWiring.newRuntimeWiring();
        ...
        return TestUtil.schemaByInputFile(schemaPath, runtimeWiring.build());
    }

    public static GraphQLSource getGraphQLByDataFetcherMap(
        Map<String, Map<String, DataFetcher>> dataFetcherInfoMap) {
        return getGraphQLByDataFetcherMap(dataFetcherInfoMap, DefaultConfig.newConfig().build());
    }

    public static GraphQLSource getGraphQLByDataFetcherMap(
        Map<String, Map<String, DataFetcher>> dataFetcherInfoMap, DefaultConfig config) {
    ...
    public static GraphQLSchema getSchemaByDataFetcherMap(
        Map<String, Map<String, DataFetcher>> dataFetcherInfoMap) {
        RuntimeWiring.Builder runtimeWiring = RuntimeWiring.newRuntimeWiring();
    ...
    public static Map<String, Map<String, DataFetcher>> defaultDataFetcherInfo() {
        ...
        couponFieldFetchers.put("couponList", async(couponListDataFetcher));
